        ...
        <tr>
          ...
          <td>
              <div class="admin-actions">
                  <button class="admin-action-btn points" onclick="window.adminAddPoints(${userId}, '${encodeURIComponent(normalizedEmail).replace(/\\/g, "\\\\").replace(/'/g, "\\'")}')">+Points</button>
...
function escapeHtml(value) {
  ...
  const authenticatedUser = user || users.find(
  ...
  if (authenticatedUser) {
    ...
    const userIndex = users.findIndex(u => Number(u.id) === Number(authenticatedUser.id));
    const normalizedUser = normalizeStoredUser(authenticatedUser, authenticatedUser.id);
