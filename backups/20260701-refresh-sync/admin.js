...
function adminSetPassiveRate(userId, rateValue) {
  if (!assertAdminDashboardAccess()) return;
  if (getCurrentUserRole() !== 'admin') { showNotification('Only admin can edit passive rate.', { type: 'error' }); renderAdminDashboard(false); return; }
  const parsed = Number(rateValue);
  if (!Number.isFinite(parsed) || parsed <= 0) { showNotification('Passive rate must be greater than 0.', { type: 'error' }); return; }
  const users = getStoredUsersSafe();
  const userIndex = findUserIndexById(users, userId);
  if (userIndex === -1) { showNotification('User not found.', { type: 'error' }); renderAdminDashboard(false); return; }
  users[userIndex].passiveRate = parsed;
  users[userIndex].updatedAt = Date.now();
  users[userIndex].lastActiveAt = Date.now();
  setStoredUsers(users);
  upsertUserInCloud(users[userIndex]);
  syncCurrentSessionIfNeeded(users[userIndex]);
  renderAdminDashboard(false);
  showNotification(`Passive rate updated for ${users[userIndex].email}.`, { type: 'success' });
}

function adminSetFruitCount(userId, fruitValue) {
  if (!assertAdminDashboardAccess()) return;
  if (getCurrentUserRole() !== 'admin') { showNotification('Only admin can edit fruit count.', { type: 'error' }); renderAdminDashboard(false); return; }
  const parsed = Math.floor(Number(fruitValue));
  if (!Number.isFinite(parsed) || parsed < 0) { showNotification('Fruit count must be 0 or greater.', { type: 'error' }); return; }
  const users = getStoredUsersSafe();
  const userIndex = findUserIndexById(users, userId);
  if (userIndex === -1) { showNotification('User not found.', { type: 'error' }); renderAdminDashboard(false); return; }
  users[userIndex].fruitCount = parsed;
  users[userIndex].updatedAt = Date.now();
  users[userIndex].lastActiveAt = Date.now();
  setStoredUsers(users);
  upsertUserInCloud(users[userIndex]);
  syncCurrentSessionIfNeeded(users[userIndex]);
  renderAdminDashboard(false);
  showNotification(`Fruit count updated for ${users[userIndex].email}.`, { type: 'success' });
}

function adminSetPointsForFruit(userId, pointsValue) {
  if (!assertAdminDashboardAccess()) return;
  if (getCurrentUserRole() !== 'admin') { showNotification('Only admin can edit fruit progress.', { type: 'error' }); renderAdminDashboard(false); return; }
  const parsed = Math.floor(Number(pointsValue));
  if (!Number.isFinite(parsed) || parsed < 0 || parsed > 99) { showNotification('Fruit points must be between 0 and 99.', { type: 'error' }); return; }
  const users = getStoredUsersSafe();
  const userIndex = findUserIndexById(users, userId);
  if (userIndex === -1) { showNotification('User not found.', { type: 'error' }); renderAdminDashboard(false); return; }
  users[userIndex].pointsForFruit = parsed;
  users[userIndex].updatedAt = Date.now();
  users[userIndex].lastActiveAt = Date.now();
  setStoredUsers(users);
  upsertUserInCloud(users[userIndex]);
  syncCurrentSessionIfNeeded(users[userIndex]);
  renderAdminDashboard(false);
  showNotification(`Fruit progress updated for ${users[userIndex].email}.`, { type: 'success' });
}

function adminSetMaxBloomReached(userId, isReached) {
  if (!assertAdminDashboardAccess()) return;
  if (getCurrentUserRole() !== 'admin') { showNotification('Only admin can edit bloom state.', { type: 'error' }); renderAdminDashboard(false); return; }
  const users = getStoredUsersSafe();
  const userIndex = findUserIndexById(users, userId);
  if (userIndex === -1) { showNotification('User not found.', { type: 'error' }); renderAdminDashboard(false); return; }
  users[userIndex].maxBloomReached = Boolean(isReached);
  users[userIndex].updatedAt = Date.now();
  users[userIndex].lastActiveAt = Date.now();
  setStoredUsers(users);
  upsertUserInCloud(users[userIndex]);
  syncCurrentSessionIfNeeded(users[userIndex]);
  renderAdminDashboard(false);
  showNotification(`Bloom state updated for ${users[userIndex].email}.`, { type: 'success' });
}

window.adminSetPassiveRate = adminSetPassiveRate;
window.adminSetFruitCount = adminSetFruitCount;
window.adminSetPointsForFruit = adminSetPointsForFruit;
window.adminSetMaxBloomReached = adminSetMaxBloomReached;
...
async function renderAdminDashboard(syncFromCloud = false) {
  ...
  const totalFruitCount = safeUsers.reduce((sum, user) => sum + Math.floor(Number(user.fruitCount ?? 0) || 0), 0);
  const passiveRateTotal = safeUsers.reduce((sum, user) => sum + (Number(user.passiveRate ?? 1) || 1), 0);
  const averagePassiveRate = totalUsers > 0 ? passiveRateTotal / totalUsers : 1;
  const bloomReachedUsers = safeUsers.filter(user => Boolean(user.maxBloomReached)).length;
  ...
  const passiveRateAvgEl = document.getElementById('adminPassiveRateAvg');
  const totalFruitCountEl = document.getElementById('adminTotalFruitCount');
  const bloomReachedCountEl = document.getElementById('adminBloomReachedCount');
  const roleAdminSummaryEl = document.getElementById('adminRoleSummaryAdmin');
  const roleModeratorSummaryEl = document.getElementById('adminRoleSummaryModerator');
  ...
  if (passiveRateAvgEl) passiveRateAvgEl.textContent = `${averagePassiveRate.toFixed(1)}x`;
  if (totalFruitCountEl) totalFruitCountEl.textContent = String(totalFruitCount);
  if (bloomReachedCountEl) bloomReachedCountEl.textContent = String(bloomReachedUsers);
  if (roleAdminSummaryEl) roleAdminSummaryEl.textContent = String(totalAdmins);
  if (roleModeratorSummaryEl) roleModeratorSummaryEl.textContent = String(totalModerators);
  ...
  tbody.innerHTML = sortedUsers.map(user => {
    ...
    const passiveRateValue = Number(user.passiveRate ?? 1) || 1;
    const fruitCountValue = Math.floor(Number(user.fruitCount ?? 0) || 0);
    const pointsForFruitValue = Math.floor(Number(user.pointsForFruit ?? 0) || 0);
    const bloomReachedValue = Boolean(user.maxBloomReached);
    // Real Login Streak
    ...
    const passiveRateControl = roleOfCurrentUser === 'admin'
      ? `<input type="number" min="0.1" step="0.1" value="${passiveRateValue.toFixed(1)}" onchange="window.adminSetPassiveRate(${userId}, this.value)" aria-label="Passive rate for ${name}">`
      : `${passiveRateValue.toFixed(1)}x`;

    const fruitControl = roleOfCurrentUser === 'admin'
      ? `<div class="admin-inline-stack"><input type="number" min="0" value="${fruitCountValue}" onchange="window.adminSetFruitCount(${userId}, this.value)" aria-label="Fruit count for ${name}"><input type="number" min="0" max="99" value="${pointsForFruitValue}" onchange="window.adminSetPointsForFruit(${userId}, this.value)" aria-label="Fruit progress for ${name}"><label class="admin-checkbox-pill"><input type="checkbox" ${bloomReachedValue ? 'checked' : ''} onchange="window.adminSetMaxBloomReached(${userId}, this.checked)"> Bloom</label></div>`
      : `<div class="admin-inline-stack"><span>${fruitCountValue} fruits</span><span>${pointsForFruitValue}/100</span><span class="admin-role-badge ${bloomReachedValue ? 'admin' : 'user'}">${bloomReachedValue ? 'Bloomed' : 'Growing'}</span></div>`;
...
function adminViewProgress(userId) {
  ...
  const progressMessage = [
    ...
    `Passive Rate: ${Number(user.passiveRate ?? 1).toFixed(1)}x`,
    `Fruits: ${Math.floor(Number(user.fruitCount ?? 0) || 0)}`,
    `Fruit Progress: ${Math.floor(Number(user.pointsForFruit ?? 0) || 0)}/100`,
    `Bloom Reached: ${Boolean(user.maxBloomReached) ? 'Yes' : 'No'}`
  ].join('\n');
...
async function adminRestoreUser(userId) {
  ...
  const bestPassiveRate = Math.max(...candidateUsers.map(u => Number(u.passiveRate ?? 1) || 1));
  const bestFruitCount = Math.max(...candidateUsers.map(u => Math.floor(Number(u.fruitCount ?? 0) || 0)));
  const bestPointsForFruit = Math.max(...candidateUsers.map(u => Math.floor(Number(u.pointsForFruit ?? 0) || 0)));
  const bestBloomReached = candidateUsers.some(u => Boolean(u.maxBloomReached));
  ...
  users[userIndex] = {
    ...
    passiveRate: bestPassiveRate,
    fruitCount: bestFruitCount,
    pointsForFruit: bestPointsForFruit,
    maxBloomReached: bestBloomReached,
...
async function restoreUserLoginStreaksFromBackup() {
  ...
  for (let i = 0; i < users.length; i++) {
    ...
    const bestPassiveRate = Math.max(...candidateUsers.map(x => Number(x.passiveRate ?? 1) || 1));
    const bestFruitCount = Math.max(...candidateUsers.map(x => Math.floor(Number(x.fruitCount ?? 0) || 0)));
    const bestPointsForFruit = Math.max(...candidateUsers.map(x => Math.floor(Number(x.pointsForFruit ?? 0) || 0)));
    const bestBloomReached = candidateUsers.some(x => Boolean(x.maxBloomReached));
