...
function clearBrowserCookies() {
  try {
    ...
    console.warn('Failed to clear cookies during refresh.', e);
  }
}
...
async function clearAppCaches() {
  try {
    ...
    console.warn('Failed to clear Cache API entries.', e);
  }
...
async function runStartupRefreshCheck() {
  const previousVersion = localStorage.getItem(APP_BUILD_VERSION_KEY);
  if (previousVersion === APP_BUILD_VERSION) return false;
  ...
  try {
    if (persistedCurrentUser) {
      ...
      if (parsedCurrentUser?.email) {
        await upsertUserInCloud(parsedCurrentUser);
      }
    }
  } catch (e) {
    console.warn('Failed to sync current user before refresh.', e);
  }
  ...
  try {
    ...
    console.warn('Failed to sync stored users before refresh.', e);
  }
  ...
  if (persistedCurrentUser) localStorage.setItem('currentUser', persistedCurrentUser);
  if (persistedUsers) localStorage.setItem('users', persistedUsers);
...
window.addEventListener('DOMContentLoaded', function() {
  ...
  runStartupRefreshCheck().then(didReload => {
    if (!didReload) initializeApp();
  });
});
