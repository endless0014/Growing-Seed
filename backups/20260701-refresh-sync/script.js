...
function haveCloudUserStateDifferences(baseUser, incomingUser) {
  ...
  // Check seed data differences
  const baseSeedState = JSON.stringify(baseUser.activeSeed || null);
  const incomingSeedState = JSON.stringify(incomingUser.activeSeed || null);
  if (baseSeedState !== incomingSeedState) {
    return true;
  }

  const baseSeedHistory = JSON.stringify(baseUser.seedHistory || []);
  const incomingSeedHistory = JSON.stringify(incomingUser.seedHistory || []);
  if (baseSeedHistory !== incomingSeedHistory) {
    return true;
  }
...
async function upsertUserInCloud(user) {
  ...
  try {
    ...
    await userDoc.update({ taskCompletions, dailyLoginState, activeSeed, seedHistory });
